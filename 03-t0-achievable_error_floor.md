...
*Draft 0.2. Closes the audit `07-pi_adversarial_audit_of_04.md` in full and consolidates the multiclass companion `04-t1_1_simplex_rigidity.md` into this paper as §4 (`04` retired to a one-line redirect). Audit items closed during the consolidation: M-1 (atomless realizability lifted from parenthetical into Theorem 2$'$ hypothesis with explicit extension-by-density clause); M-2 ($c_\varphi<\infty$ regularity stated as upper-bracket hypothesis in §4.1 with entropy/Gini verifications); m-1 (§4.3 'coarsest functional' framing replaced with level-set coincidence characterization); m-2 (§4.3 'dimension' line replaced with kink-locus widening to a $(k-2)$-dimensional skeleton); m-3 (§4.2 Step 1 highlighted as the operative level-set-refinement constraint with forward pointer to the Gini/entropy counterexamples); m-4 (§4.2 Step 2 bounded-Jensen footnote added to rule out Hamel-basis pathology); m-5 (resolved structurally by `04` retirement); m-6 (§4.4 Theorem 1$'$ refinement-monotonicity transfer to the simplex stated explicitly). Also reflected in this draft: contribution list updated to four items (Item 3 = multiclass extension); §1.2 audit augmented with the multiclass-surrogate-calibration credit (Tewari–Bartlett 2007, Ramaswamy–Agarwal 2012, Pires–Szepesvári 2016); §8 Open Problem 1a entry redirected to §4 (no longer cites the retired companion); abstract notes the multiclass reach. The previously-closed Draft 0.1 audit items (C-1, C-2, M-1, M-2, M-3, M-4, M-5, m-1–m-6 of `05`) remain closed and are re-verified by the consolidation. Open: OP1b (soft cell assignments), OP2 (quantitative over-smoothing rate), OP3 (gap amplification), the empirical companion, the closed-form $\ell_\varphi$ for entropy and Gini, and the mechanization kernel (deferred per `09-mechanization_strategy.md`). The execution log is `08-draft0.2_commitology.md`.*
