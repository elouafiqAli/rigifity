...
*Draft 0.3. Closes the external reviewer audit `10-external_reviewer_audit_of_draft0.2.md` in full. Audit items addressed: **C-1** (Theorem 5 statement and proof re-installed at the top of §6, restoring the C-2 closure of `05` that was silently regressed by the Draft 0.1 §6 rewrite; preserves the for-every-labeling quantifier on direction (b) and the operational-reading remark); **M-1** (§4.2 closing remark corrected to acknowledge that the simplex proof is not 'verbatim' the binary proof — the binary case collapses to Step 1 alone via symmetry, while the simplex case genuinely uses all three steps because $\varphi$ is no longer a function of $R$); **M-2** (the over-smoothing bound $\delta^{(L)}\le C\lambda_2^L$ is now framed as the standard analysis of [Oono–Suzuki 2020; Cai–Wang 2020; Rusch et al.\ 2023], with explicit constants deferred to OP2); **m-1** (§2's WL-equivalence minimum separating distance renamed $\delta_{\mathrm{WL}}$ to disambiguate from §6's architecture-equivalence $\delta^{(L)}$); **m-2** (Cor 7's Hoeffding accuracy parameter renamed $\varepsilon\to\alpha$ to disambiguate from the partition-resolution $\varepsilon$); **m-3** (the 'Lipschitz/quantization bridge' jargon in the §6 opener replaced with the explicit Voronoi comparison $\Pi^{(L)}_\varepsilon=\Pi^{(L)}_{\mathcal A}$ for $\varepsilon<\delta^{(L)}$); **m-4** (Pires moved into alphabetical position in the references); **m-5** (§1.1 item 2 updated to note Theorem 5 lifts to multiclass via Theorem 1$'$); **m-6** (the JK join expression disambiguated, $\Pi^{(L)}_{\mathcal A^{JK}}=\bigvee_{\ell\le L}\Pi^{(\ell)}_{\mathcal A^{\rm base}}$, naming the underlying base architecture and its join variant distinctly). The previously-closed audit items of `05` and `07` remain closed and are re-verified by this draft. Open: OP1b (soft cell assignments), OP2 (quantitative over-smoothing rate), OP3 (gap amplification), the empirical companion, the closed-form $\ell_\varphi$ for entropy and Gini, and the mechanization kernel (deferred per `09-mechanization_strategy.md`). The execution log is `11-draft0.3_commitology.md`.*
