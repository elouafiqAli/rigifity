...
**Soft and attention architectures, via the soft-kernel lifting.** The rigidity is a statement about the *induced conditional label distribution* $\boldsymbol\eta_i$ of each cell, and it does not care whether the cell arises from a hard partition or from a soft assignment. Combined with the soft-kernel (T9) lifting — which expresses a soft aggregator's cell masses and rates as the label distributions induced by a Markov kernel — the simplex rigidity reaches the soft/distributional architectures (GAT's attention, differentiable pooling, graph transformers) on multiclass tasks. To be precise about credit: the genuinely new theorem here is the *multiclass label* rigidity; the soft-architecture reach is this result composed with the (already-stated) soft-kernel lifting, not a separate proof.
