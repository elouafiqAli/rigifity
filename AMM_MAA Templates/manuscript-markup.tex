...
%% Author:   Ali Elouafiq (SQLI Digital Lab, Hay Riad, Rabat, Morocco)
%% Companion anonymous file: manuscript-anon.tex (title block removed; no
%% author identity anywhere, including comments).
...
%% MAA graphic baseline: keep line art readable in print conversion.
...
SQLI Digital Lab \\
Mahaj Ryad Center, 2nd Floor, Hay Riad, 10000 Rabat, Morocco \\
\texttt{aelouafiq@sqli.com}
\end{center}
...
\small\itshape
Ali Elouafiq is Senior Research Engineer and Program Manager of the
SQLI Digital Lab in Hay Riad, Rabat, Morocco. His research interests
include the theoretical foundations of machine learning, the
falsification and validation of safety-critical systems, the
calibration of surrogate losses in statistical decision theory, and
the use of proof assistants in applied research.
\end{minipage}
\end{center}
...
%% ---------------------------------------------------------------------------
\begin{abstract}
Jensen's inequality is one of analysis's workhorse estimates: for every concave
function~$G$ and random variable~$\xi$ on an interval,
$\E[G(\xi)]\le G(\E[\xi])$. Suppose equality is forced for every
...
Yes, and the proof is one line. Substitute $u_1=M$, $u_2=0$, $p=v/M$
...
hypothesis intervenes. That historical reflex is useful in the rational setting but misleading here. Classical analysis from Cauchy (1821) to
Kestelman (1947) cataloged the minimal tameness conditions that rule
the pathology out, and the conditions became standard safeguards in
functional-equation arguments\,---\,sometimes carried into the continuous-coefficient setting, where the equation itself already excludes the pathology.

This article isolates the asymmetry and makes it explicit. We catalog which
...
survives rational-weight tests and is exposed at the irrational weights at which the
equation now tests~$G$), and give a diagnostic separating the saturated-Jensen
settings in which the hypotheses are vestigial from the more common
settings in which they remain load-bearing. The same five hypotheses
...
natural equality question: when does equality hold for \emph{every} two-point
...
at least three forms in classical analysis. They are easy to conflate, and that conflation matters both for the foundational theory and for modern applied arguments in which the equation reappears.
...
That distinction is the subject of the paper.
...
The hypothesis list deserves attention: no regularity of any
kind---continuity, measurability, monotonicity, or boundedness---is
assumed of~$G$. The proof is one line: a substitution into~\eqref{eq:star} pins
...
to explain why the distinction matters.

\subsection{The historical detour and the structural moral.}
\label{ssec:detour}

Theorem~\ref{thm:main} is elementary. Its substitution
...
Section~\ref{sec:recurrence} needs it as a separable lemma. The paper's contribution is threefold:
\begin{itemize}
\item \emph{The explicit dictionary} (Section~\ref{sec:dictionary}) of
  five classical tameness hypotheses on~$G$, each sufficient to force
  affineness for~\eqref{eq:J2} but vestigial under~\eqref{eq:star},
  together with the structural reason (developed in Section~\ref{ssec:mechanism}).
...
\item \emph{The diagnostic} (Section~\ref{sec:recurrence}) for
  ...
  real-valued unknown at all (the zeroth question). In the
...
The article deliberately foregrounds one part of Jensen-equation
theory: the regime in which the continuous-coefficient form needs no
regularity assumption on the function. Kuczma's monograph~\cite{Kuczma2009}
is cited only for the classical regularity catalog, the rational-coefficient
pathology, and convex-domain variants, not for the exact
continuous-coefficient endpoint-substitution statement isolated here.
...
dictionary, the structural mechanism, and the diagnostic of applied
recurrences.
...
This pattern\,---\,a continuum built into the model retiring a
...
The order of inference matters enough to state explicitly. Under
...
regularity internally; no external regularity hypothesis is needed.
...
and the continuous-coefficient equation~\eqref{eq:star}, and ask which regularity hypotheses on~$G$ classically suffice to force affineness, and whether any such tameness assumption is needed at all. Table~\ref{tab:dictionary} lists these hypotheses side-by-side,
and Figure~\ref{fig:dictionary} draws the comparison as a load
diagram: five classical hypotheses, each on its own strong enough
...
are classically sufficient, one at a time, to force affineness for the
...
The ground line carries the point of the figure. With \textbf{no
...
The five struts tell the complementary story. For~\eqref{eq:J2},
...
dictionary's terminology, \emph{vestigial}: classically useful as a sufficient tameness route for~\eqref{eq:J2}, classically expected for~\eqref{eq:star}, but in
...
For fixed $M>0$, if the same identity held for every $p\in[0,1]$ and $G$ were additive on $\R$, then for any real $y$ one could choose an integer $n$ with $p=|y|/(nM)\le1$; additivity and sign reversal would give $G(y)=(y/M)G(M)$, forcing $\R$-linearity.
...
The structural point fits in one sentence:

\begin{quote}
\textbf{The Hamel pathology survives the rational tests but is exposed by irrational $p$; the
...
\node[align=center] at (9.5,0.45)
  {\scriptsize an irrational $p$ (here $1/\sqrt{2}$, a schematic marker): \eqref{eq:star} tests here too,\\[-1pt]
   \scriptsize and some such test exposes any non-$\R$-linear defect (Section~\ref{ssec:mechanism})};
...
equation~\eqref{eq:star} also tests the irrational weights, where some test must expose any failure of $\R$-linearity, and forecloses on the
pathology there.}
\label{fig:mechanism}
\end{figure}
...
is \emph{available}, and the regularity dictionary no longer has load to bear.
...
Theorem~\ref{thm:weak} is logically weaker than
...
Two further variations sharpen both points: how little input the conclusion needs, and what survives when the full hypothesis is cut
down.
...
is affine. The Hamel pathology is exposed by irrational weights
...
is necessarily piecewise affine\,---\,a tent-shaped profile once the endpoint and scale normalizations are fixed\,---\,with
...
$C\subseteq V$ be a nonempty convex set, and let $G\colon C\to\R$ satisfy
\[
p\,G(x_1)+(1-p)\,G(x_2) \;=\;
...
scaling device, choosing in the additivity step $t>0$ small enough that $t(\lambda+\mu)\le1$, upgrades (ii) to additivity of $\hat A$ on all
...
largely bookkeeping (compare the regularity-laden treatments of the Jensen
...
in~\eqref{eq:star} does genuine work: the rational-coefficient
...
Proposition~\ref{prop:JQ-pathology} makes sharp the dictionary:
...
is to draw that line and show how to use it. The conclusion is not
that applied authors habitually carry unnecessary hypotheses; it is
more precise and, we think, more useful: the saturated-Jensen identity
...
the four are easily mistaken for one another.
...
p\,G(u_1)+(1-p)\,G(u_2)\;=\;G\bigl(p\,u_1+(1-p)\,u_2\bigr)
\]
is, first of all, a \emph{family} of assertions indexed by the
...
Four recurring sources are worth separating.
...
This yields a simple ordered diagnostic. The \emph{zeroth question} \,---\,the name we use throughout, and a gate one must clear before any weight is examined\,---\, is
...
examined. Once this gate is cleared, the question that separates the
...
The trap, then, is \textbf{misclassification}, and it runs both ways. A derivation of one of the first three kinds can be
...
We begin on the side of the line where the classical hypotheses do real work\,---\,which, perhaps against first intuition, is where most
...
Archimedean axiom. That reading is too quick. The
...
needed to produce one. In the diagnostic's terms, this failure is not about a shortage of weights\,---\,independence is stated at every real $p$\,---\,but a
failure at the zeroth question: there is no real-valued unknown for the
...
and the hypotheses used to exclude them belong to the same regularity families represented by the five struts of Figure~\ref{fig:dictionary}. Faddeev assumed continuity of
...
There is an instructive complement, recorded in Acz\'el and
...
A third mechanism is worth naming briefly, because it too is readily
...
Now we cross the line, where the continuum of weights is
...
$H(u):=\inf_{a\in\mathcal{A}}\bigl((1-u)\ell(a,0)+u\ell(a,1)\bigr)$; the function $H$ is
...
dictionary separating clauses that the original framework states together. Reid and
...
continuum of weights can supply it. In those terms it lies past the zeroth question but is a
...
with~\eqref{eq:star}\,---\,and conflating the two is exactly the misreading the diagnostic above is built to prevent.

This is the tempting misclassification warned of above in miniature: the saturated identity
...
shape is piecewise affine; with the usual $0$--$1$ endpoint and scale normalizations, this becomes the familiar symmetric tent, whose
...
\emph{converse}: piecewise saturation on the benchmark cells pins the piecewise-affine shape; the endpoint and scale normalizations then select the tent, with affineness on each cell and continuity at the apex obtained
for free and no regularity assumed. The mixture representation is equally
concrete. Under the usual binary proper-score representation, every suitably normalized proper score's uncertainty function $G$ is recovered from the
...
$\int_0^1 T_\theta\,d\theta = u(1-u)$ with this normalization is then the Brier entropy, the case
...
\node at (11.62,1.12) {\scriptsize $u(1-u)=\int_0^1\!T_\theta\,d\theta$ (chosen norm.)};
% bold tent
...
scores~\cite{Schervish1989,EGJK2016}, and the usual binary proper-score representation writes suitably normalized uncertainty functions as mixtures $\int_0^1 T_\theta\,d\nu(\theta)$;
the dashed curve is the Brier entropy $u(1-u)$, the uniform mixture under the chosen tent normalization.
Exactness on each cell of the benchmark partition
$\{[0,\tfrac12],[\tfrac12,1]\}$ forces affineness on each cell
(Corollary~\ref{cor:piecewise}): a piecewise-affine tent shape, with the familiar symmetric tent selected by endpoint and scale normalizations, pinned with nothing
assumed at the kink.}
\label{fig:resolution}
\end{figure}

\paragraph{Two readings.}
The same reading travels. In probabilistic graphical models, a clique
...
a genuinely concave~$G$ can certify a resolution gain through a strict drop of~$U_G$, provided the refinement crosses a segment on which $G$ is strictly concave.
...
main contrast of Sections~\ref{sec:dictionary} and~\ref{sec:recurrence} is
...
coarse-graining is a finite-additivity operation rather than a direct continuum of real convex mixtures\,---\,the first source.
Finite additivity yields homogeneity only over the nonnegative rationals, so the rational-to-real step must be purchased with a regularity hypothesis. Each classical derivation pays in its own coin. Gleason's original projection-based theorem~\cite{Gleason1957} uses nonnegativity (for $d \ge 3$), as does Busch's POVM version~\cite{Busch2003} (for all $d \ge 2$); Caves, Fuchs, Manne, and Renes~\cite{CFMR2004} use positivity or an explicit continuity argument. The Cauchy-equation treatment of Wright and Weigert~\cite{WrightWeigert2019} (Theorem~1, with a corrected proof in~\cite{WrightWeigert2020}) makes the dependence sharpest: a nonlinear finitely additive frame function built on a Hamel basis can be neither bounded on either side, nor continuous at zero, nor Lebesgue measurable\,---\,the catalog of Figure~\ref{fig:dictionary} rediscovered inside physics. State
...
predicts which route needs a rational-to-real bridge, and the derivations cited above supply such a bridge through positivity, boundedness, or continuity-type hypotheses.
...
load-bearing case above (the entropy
characterization, a Cauchy relative) beside the vestigial case
(the atomless calibration computation,
...
\midrule
\textbf{Continuity}
  & \textbf{Load-bearing} (Faddeev~\cite{Faddeev1957}).
  & \textbf{Vestigial}\,---\,a conclusion (Corollary~\ref{cor:regularity}). \\
\midrule
\textbf{Measurability}
  & \textbf{Load-bearing} (Lee~\cite{Lee1964}).
  & \textbf{Vestigial}\,---\,same. \\
\midrule
\textbf{Monotonicity}
  & \textbf{Load-bearing} (Kendall~\cite{Kendall1964}, Borges~\cite{Borges1967}).
  & \textbf{Vestigial}\,---\,same. \\
\midrule
\textbf{Boundedness on a set of positive measure}
  & \textbf{Load-bearing} (Diderrich~\cite{Diderrich1975}).
  & \textbf{Vestigial}\,---\,same. \\
\midrule
\textbf{Boundedness on the interval}
  & \textbf{Load-bearing} (a fortiori).
  & \textbf{Vestigial}\,---\,same. \\
\midrule
\textbf{None}
...
same five hypotheses serve as load-bearing regularity routes for the \emph{binary} ($n{=}2$)
...
The recurrence worth articulating is less a habit of carrying unnecessary hypotheses than the ease of misreading which of the
...
and only the last retires the regularity hypotheses. The zeroth question
precedes it: absent an existence axiom there is no real-valued
unknown, and the question of weights never arises. The diagnostic
above is offered as a way to tell
...
asymmetry above holds; the continuous-coefficient
equation~\eqref{eq:star} forecloses on it there.} Why does this material
deserve an article in the \textsc{Monthly} as an expository matter?
...
make visible. That scaffolding is the paper's expository purpose.

\subsection*{A note on provenance.}

The present observation reached the author through an unconventional
route: a formalization project on a separate topic, in which the
...
A literature search then placed the substitution technique in its folklore context: Acz\'el's \cite[\S2.1.4]{Aczel1966} proves a dyadic precursor
...
The lesson is
broader: a defensive regularity hypothesis is not free, and a
