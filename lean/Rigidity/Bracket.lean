...
`bar phi(Π) ≤ phi(eps*(Π))` (lower endpoint) and `eps*(Π) ≤ c_phi · bar phi(Π)`
...
/-- Refinement order on partitions: `Π'` refines `Π` iff every cell of `Π'`
    lies inside a cell of `Π`. -/
def Refines {α : Type*} [MeasurableSpace α] (Π' Π : FinitePartition α) : Prop :=
  ∀ c' ∈ Π'.cells, ∃ c ∈ Π.cells, c' ⊆ c

@[inherit_doc] infix:50 " ⪰ " => Refines

variable {α : Type*} [MeasurableSpace α] (μ : Measure α)

/-- Mass of a cell under measure `μ`. -/
noncomputable def cellMass (Π : FinitePartition α) (c : Set α) : ℝ≥0∞ := μ c
...
noncomputable def cellRate (f : α → Bool) (_Π : FinitePartition α) (c : Set α) : ℝ := by
  -- Phase B target: `(μ ({x | f x = true} ∩ c)).toReal / (μ c).toReal`,
  -- with the division-by-zero convention.
  exact sorry

/-- Partition-restricted Bayes risk: `ε*(Π) = Σᵢ pᵢ · min(ηᵢ, 1 - ηᵢ)`.
    Brick: `D-eps*`. Manuscript: §2 ¶1. -/
noncomputable def epsilonStar (f : α → Bool) (Π : FinitePartition α) : ℝ :=
  ∑ c ∈ Π.cells, (cellMass μ Π c).toReal *
    min (cellRate μ f Π c) (1 - cellRate μ f Π c)

/-- Partition functional `bar φ(Π) = Σᵢ pᵢ · φ(ηᵢ)`.
    Brick: `D-bphi`. Manuscript: §2 ¶1. -/
noncomputable def barPhi (φ : ℝ → ℝ) (f : α → Bool) (Π : FinitePartition α) : ℝ :=
  ∑ c ∈ Π.cells, (cellMass μ Π c).toReal * φ (cellRate μ f Π c)
...
/-- **Binary bracket — lower endpoint** (inverse-free form).
    ...
    gives `Σ pᵢ φ(qᵢ) ≤ φ(Σ pᵢ qᵢ)`, i.e. `bar φ(Π) ≤ φ(ε*(Π))`.
    Applying `φ⁻¹` (which exists by `StrictMonoOn`) recovers `φ⁻¹(bar φ) ≤ ε*`.
    Brick: `T-bracket` (lower half). -/
theorem bracket_lower (φ : ℝ → ℝ) (h : NormalizedScore φ)
    (f : α → Bool) (Π : FinitePartition α) :
    barPhi μ φ f Π ≤ φ (epsilonStar μ f Π) := by sorry

/-- **Binary bracket — upper endpoint** `ε*(Π) ≤ c_φ · bar φ(Π)`.
    Via the pointwise inequality `η ≤ c_φ · φ(η)` on `(0, 1/2]`, aggregated.
    Brick: `T-bracket` (upper half). -/
theorem bracket_upper (φ : ℝ → ℝ) (h : NormalizedScore φ)
    (f : α → Bool) (Π : FinitePartition α) :
    epsilonStar μ f Π ≤ cPhi φ * barPhi μ φ f Π := by sorry
