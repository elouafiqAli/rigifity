...
REMOTE_LIBRARY_DIR = "/workspace/library"
...
library_mount = modal.Mount.from_local_dir(str(LIBRARY_DIR), remote_path=REMOTE_LIBRARY_DIR)
...
def extract_one(
    ...
    source_path = Path(REMOTE_LIBRARY_DIR) / pdf_filename
    if not source_path.exists():
        raise FileNotFoundError(f"Input PDF not found inside Modal mount: {source_path}")
...
def run(
    ...
    payloads = extract_one.starmap(
        [(name, page_batch_size, num_threads, 1800.0) for name in pdf_names],
        order_outputs=False,
    )
