%% cover-letter-accepted.tex  —  The American Mathematical Monthly
...
The manuscript shows that the continuous-coefficient Jensen equation
...
endpoints. The manuscript pairs this one-line observation with
...
operators\,---\,shows the asymmetry at work in a single modern literature: Gleason-type derivations of the Born rule spend positivity or regularity assumptions of the same Cauchy--Hamel kind on the measurement side, while the
...
to verify in an afternoon, yet modern applied work spans both
...
supplies the diagnostic that tells them apart, and its closing note on provenance recounts how the author was led to it by an
instance of the rarer error of adding a hypothesis that was not
needed. This is a natural fit for the \textit{Monthly}: an expository article that
...
under a continuity hypothesis. The manuscript cites Kuczma only for the classical Cauchy--Jensen regularity catalog, the rational-coefficient pathology, and convex-domain background, not as an explicit source for the exact continuous-coefficient endpoint-substitution statement isolated here. The precise
...
Theorem~1 itself but its explicit framing: the dictionary
...
a saturated identity. In that sense, the article contrasts fully developed classical Cauchy--Jensen routines with a continuous-coefficient use that often remains implicit in applied arguments and is made explicit here. The closing note records that the observation
