...
operators\,---\,shows the asymmetry \ReviewDel{alive in a single modern
literature: Gleason-type derivations of the Born rule spend exactly
the dictionary's hypotheses}\ReviewAdd{at work in a single modern literature: Gleason-type derivations of the Born rule spend positivity or regularity assumptions of the same Cauchy--Hamel kind} on the measurement side, while the
...
a saturated identity. \ReviewAdd{In that sense, the article contrasts fully developed classical Cauchy--Jensen routines with a continuous-coefficient use that often remains implicit in applied arguments and is made explicit here.} \ReviewDel{A closing note records}\ReviewAdd{The closing note records} that the observation
