...
operators\,---\,shows the asymmetry alive in a single modern
literature: Gleason-type derivations of the Born rule \ReviewDel{spend exactly
the dictionary's hypotheses}\ReviewAdd{spend positivity or regularity assumptions of the same Cauchy--Hamel kind} on the measurement side, while the
...
to verify in an afternoon, and yet modern applied work spans both
...
supplies the diagnostic that tells them apart, and (in its closing
note on provenance) recounts how the author was led to it by an
instance of the rarer error of adding a hypothesis that was not
needed. The Monthly is the natural home for an expository article that
...
not require regularity assumptions on the function}. \ReviewAdd{The manuscript cites Kuczma for the classical Cauchy--Jensen regularity catalog, rational-coefficient pathology, and convex-domain background; it does not present Kuczma as an explicit source for the exact continuous-coefficient endpoint-substitution statement isolated here.} The precise
...
a saturated identity. \ReviewAdd{In that sense, the article contrasts the fully developed classical Cauchy--Jensen routines with a continuous-coefficient use that appears more indirectly across applied arguments and is made explicit here.} A closing note records that the observation
