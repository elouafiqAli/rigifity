%% =============================================================================
%% manuscript-accepted.tex — The American Mathematical Monthly (with author details)
...
the interval and all weights $p\in[0,1]$? Does this force $G$ to be affine?
...
survives rational-weight tests and is exposed by the irrational weights that the equation now
...
\item \emph{The explicit dictionary} (Section~\ref{sec:dictionary}) of
  ...
  together with the structural reason: Hamel examples survive the
  rational tests and are exposed at irrational~$p$.
...
\node[align=center] at (9.5,0.45)
  {\scriptsize an irrational $p$ ($1/\sqrt{2}$schematic marker): \eqref{eq:star} tests here too,\\[-1pt]
...
This yields a simple ordered diagnostic. The \emph{zeroth} question is
...
examined. Once that threshold question is cleared, the question that separates the
...
Section~\ref{ssec:diagnostic}, this failure is not about a shortage of weights\,---\,independence is stated at every real $p$\,---\,but a
failure at the zeroth threshold: there is no real-valued unknown for the
...
This is the tempting misclassification of
Section~\ref{ssec:diagnostic} in miniature: the saturated identity
...
and only the last retires the regularity hypotheses. A zeroth threshold
...
of Section~\ref{ssec:diagnostic} is offered as a way to tell
...
Hamel pathology survives rational tests and is exposed by irrational~$p$; the continuous-coefficient
