%% =============================================================================
%% manuscript.tex — The American Mathematical Monthly (with author details)
...
\usepackage[normalem]{ulem}     % review markup: strike-through deletions

%% --- Combined review markup: blue insertions, red struck deletions ---
...
\textbf{\large How Real Mixing Weights Retire Regularity} \\
\vspace{30px}
Ali Elouafiq \\
...
%% ---------------------------------------------------------------------------
\begin{abstract}
Jensen's inequality is \ReviewDel{a workhorse of analysis}\ReviewAdd{one of analysis's workhorse estimates}: for every concave
function~$G$ and random variable~$\xi$ on an interval,
$\E[G(\xi)]\le G(\E[\xi])$. \ReviewDel{What if equality is forced}\ReviewAdd{Suppose equality is forced} for every
two-point distribution\,---\,that is,
$p\,G(u_1)+(1-p)\,G(u_2)=G(p\,u_1+(1-p)\,u_2)$ for all $u_1,u_2$ in
the interval and all weights $p\in[0,1]$? \ReviewDel{Must}\ReviewAdd{Does this force} $G$ \ReviewAdd{to} be affine?

\ReviewDel{Yes.}\ReviewAdd{Yes, and the proof is one line.} \ReviewDel{The proof is one line\,---\,substitute}\ReviewAdd{Substitute} $u_1=M$, $u_2=0$, $p=v/M$
...
hypothesis intervenes. \ReviewAdd{That historical reflex is useful in the rational setting but misleading here.} Classical analysis from Cauchy (1821) to
Kestelman (1947) cataloged the minimal tameness conditions that rule
the pathology out, and the conditions became \ReviewDel{standard baggage}\ReviewAdd{standard safeguards} in
functional-equation arguments\,---\,\ReviewDel{carried even where the
continuous-coefficient equation is in play and none of the baggage is
needed}\ReviewAdd{sometimes carried into the continuous-coefficient setting, where the equation itself already excludes the pathology}.

\ReviewDel{This expository article makes}\ReviewAdd{This article isolates} the asymmetry \ReviewAdd{and makes it explicit}. We catalog which
...
\ReviewDel{lives at irrational weights, exactly where}\ReviewAdd{survives rational-weight tests and is exposed by the irrational weights that} the equation now
tests~$G$), and give a diagnostic separating the saturated-Jensen
settings \ReviewDel{where}\ReviewAdd{in which} the hypotheses are vestigial from the more common
settings \ReviewDel{where}\ReviewAdd{in which} they remain load-bearing. The same five hypotheses
...
\ReviewDel{immediate follow-up}\ReviewAdd{natural equality question}: when does equality hold for \emph{every} two-point
...
at least three forms in classical analysis. \ReviewDel{The three forms are easily
...
the equation surfaces.}\ReviewAdd{They are easy to conflate, and that conflation matters both for the foundational theory and for modern applied arguments in which the equation reappears.}
...
\ReviewDel{This is}\ReviewAdd{That distinction is} the subject of the paper.
...
The hypothesis list deserves \ReviewDel{a second look}\ReviewAdd{attention}: no regularity of any
kind---continuity, measurability, monotonicity, or boundedness---is
assumed of~$G$. The proof is one line: a substitution into~\eqref{eq:star} \ReviewDel{that pins}\ReviewAdd{pins}
...
to \ReviewDel{articulate what it means}\ReviewAdd{explain why the distinction matters}.

\subsection{The historical detour and the structural moral.}
\label{ssec:detour}

Theorem~\ref{thm:main} is elementary. \ReviewDel{The underlying}\ReviewAdd{Its} substitution
...
Section~\ref{sec:recurrence} needs it as a separable lemma. \ReviewDel{The
contribution of the present paper is in three parts:}\ReviewAdd{The paper's contribution is threefold:}
\begin{itemize}
\item \emph{The explicit dictionary} (Section~\ref{sec:dictionary}) of
  ...
  boundedness on the interval\,---\,each of which is \ReviewDel{required}\ReviewAdd{classically sufficient} to force
  affineness for~\eqref{eq:J2}\ReviewAdd{, while some tameness condition is needed to exclude Hamel examples,} but becomes \emph{vestigial}
  under~\eqref{eq:star}, together with the structural mechanism (the
  Hamel pathology \ReviewDel{lives at}\ReviewAdd{passes the rational tests but is exposed by} irrational~$p$, exactly where
...
\ReviewDel{Stylistically, this article foregrounds}\ReviewAdd{The article deliberately foregrounds} one part of Jensen-equation
theory: the regime in which the continuous-coefficient form needs no
regularity assumption on the function. \ReviewDel{That emphasis is ours; Kuczma's
...
regularity-free structural theory.}\ReviewAdd{Kuczma's monograph~\cite{Kuczma2009} remains in the paper as background for the classical Cauchy--Jensen regularity catalog, the rational-coefficient pathology, and Jensen equations on convex domains; the manuscript does not rely on it as an explicit source for the precise continuous-coefficient, regularity-free endpoint-substitution statement isolated here.}
...
dictionary, the structural mechanism, and the \ReviewDel{survey}\ReviewAdd{diagnostic} of applied
recurrences.
...
\ReviewDel{The pattern}\ReviewAdd{This pattern}\,---\,a continuum built into the model retiring a
...
The order of inference matters \ReviewDel{and is worth stating explicitly}\ReviewAdd{enough to state explicitly}. Under
...
regularity \ReviewDel{from within}\ReviewAdd{internally}; no external regularity hypothesis is needed.
...
We place \ReviewDel{side-by-side}\ReviewAdd{side by side} the discrete-coefficient equation~\eqref{eq:J2}
and the continuous-coefficient equation~\eqref{eq:star}, and ask \ReviewDel{which
regularity hypotheses on~$G$ are \emph{required} to force affineness
for each}\ReviewAdd{which regularity hypotheses on~$G$ classically suffice to force affineness, and whether any such tameness assumption is needed at all}. Table~\ref{tab:dictionary} lists these hypotheses side-by-side,
and Figure~\ref{fig:dictionary} draws the comparison as a load
diagram: five classical hypotheses, each \ReviewDel{individually}\ReviewAdd{on its own} strong enough
...
are classically \ReviewDel{required}\ReviewAdd{sufficient, one at a time,} to force affineness for the
...
The ground line \ReviewDel{is the point of}\ReviewAdd{carries the point of} the figure. With \textbf{no
...
The five struts tell \ReviewDel{a complementary story}\ReviewAdd{the complementary story}. For~\eqref{eq:J2},
...
dictionary's terminology, \emph{vestigial}: \ReviewDel{classically required
for~\eqref{eq:J2}}\ReviewAdd{classically useful as a sufficient tameness route for~\eqref{eq:J2}}, classically expected for~\eqref{eq:star}, but in
...
mechanism is \ReviewDel{direct and illuminating}\ReviewAdd{simple}.
...
\ReviewAdd{For fixed $M>0$, if the same identity held for every $p\in[0,1]$ and $G$ were additive on $\R$, then for any real $y$ one could choose an integer $n$ with $p=|y|/(nM)\le1$; additivity and sign reversal would give $G(y)=(y/M)G(M)$, forcing $\R$-linearity.}
...
The structural point \ReviewDel{is}\ReviewAdd{fits in} one sentence:

\begin{quote}
\textbf{The Hamel pathology \ReviewDel{lives at}\ReviewAdd{survives the rational tests but is exposed by} irrational $p$; the
...
\node[align=center] at (9.5,0.45)
  {\scriptsize an irrational $p$ (\ReviewDel{here }$1/\sqrt{2}$\ReviewAdd{schematic marker}): \eqref{eq:star} tests here too,\\[-1pt]
   \scriptsize \ReviewDel{and the pathology dies}\ReviewAdd{and some such test exposes any non-$\R$-linear defect} (Section~\ref{ssec:mechanism})};
...
equation~\eqref{eq:star} also tests the irrational weights, where \ReviewDel{the
$\R$-linearity defect must show itself}\ReviewAdd{some test must expose any failure of $\R$-linearity}, and forecloses on the
pathology there.}
\label{fig:mechanism}
\end{figure}
...
is \emph{available}, and the \ReviewDel{entire dictionary collapses}\ReviewAdd{regularity dictionary no longer has load to bear}.
...
Theorem~\ref{thm:weak} is \ReviewDel{\emph{in principle}}\ReviewAdd{logically} weaker than
...
Two further variations sharpen \ReviewDel{the picture of how little input the
conclusion needs, and of what survives when}\ReviewAdd{both points: how little input the conclusion needs, and what survives when} the full hypothesis is cut
down.
...
is affine. The Hamel pathology \ReviewDel{lives at}\ReviewAdd{is exposed by} irrational weights
...
is necessarily piecewise affine\,---\,\ReviewDel{the tent shape}\ReviewAdd{a tent-shaped profile once the endpoint and scale normalizations are fixed}\,---\,with
...
$C\subseteq V$ be a \ReviewAdd{nonempty} convex set, and let $G\colon C\to\R$ satisfy
\[
p\,G(x_1)+(1-p)\,G(x_2) \;=\;
...
scaling device\ReviewAdd{, choosing in the additivity step $t>0$ small enough that $t(\lambda+\mu)\le1$,} upgrades (ii) to additivity of $\hat A$ on all
...
\ReviewDel{bookkeeping}\ReviewAdd{largely bookkeeping} (compare the regularity-laden treatments of the Jensen
...
in~\eqref{eq:star} \ReviewDel{is doing}\ReviewAdd{does} genuine work: the rational-coefficient
...
Proposition~\ref{prop:JQ-pathology} \ReviewDel{sharpens}\ReviewAdd{makes sharp} the dictionary:
...
is to \ReviewDel{draw that line precisely and to walk it}\ReviewAdd{draw that line and show how to use it}. The conclusion is not
that applied authors habitually carry unnecessary hypotheses; it is
\ReviewDel{subtler}\ReviewAdd{more precise} and, we think, more useful: the saturated-Jensen identity
...
the four are \ReviewDel{easy to mistake for one another}\ReviewAdd{easily mistaken for one another}.
...
p\,G(u_1)+(1-p)\,G(u_2)\;=\;G\bigl(p\,u_1+(1-p)\,u_2\bigr)
\]
is, \ReviewDel{before anything else,}\ReviewAdd{first of all,} a \emph{family} of assertions indexed by the
...
\ReviewDel{Four sources recur.}\ReviewAdd{Four recurring sources are worth separating.}
...
This yields \ReviewDel{an ordered diagnostic}\ReviewAdd{a simple ordered diagnostic}. The \emph{zeroth} question is
...
examined. Once that \ReviewDel{gate}\ReviewAdd{threshold question} is cleared, the question that separates the
...
The trap, then, is \textbf{misclassification}, and it runs \ReviewDel{in both
directions}\ReviewAdd{both ways}. A derivation of one of the first three kinds can be
...
We begin on the side of the line where the classical hypotheses \ReviewDel{earn
their keep}\ReviewAdd{do real work}\,---\,which, perhaps against first intuition, is where most
...
Archimedean axiom. \ReviewDel{The temptation should be resisted.}\ReviewAdd{That reading is too quick.} The
...
Section~\ref{ssec:diagnostic}, this failure is not \ReviewDel{a deficit of
weights}\ReviewAdd{about a shortage of weights}\,---\,independence is stated at every real $p$\,---\,but a
failure at the \ReviewDel{zeroth gate}\ReviewAdd{zeroth threshold}: there is no real-valued unknown for the
...
and the hypotheses used to exclude them \ReviewDel{are \emph{exactly the five
struts of Figure~\ref{fig:dictionary}}}\ReviewAdd{belong to the same regularity families represented by the five struts of Figure~\ref{fig:dictionary}}. Faddeev assumed continuity of
...
There is \ReviewDel{a striking complement}\ReviewAdd{an instructive complement}, recorded in Acz\'el and
...
A third mechanism is worth naming briefly, because it too is \ReviewDel{easily}\ReviewAdd{readily}
...
Now \ReviewDel{the other side of the line}\ReviewAdd{we cross the line}, where the continuum of weights is
...
\ReviewDel{$H(u):=\inf_{a\in\mathcal{A}}\E[\ell(a,Y)\mid \eta=u]$}\ReviewAdd{$H(u):=\inf_{a\in\mathcal{A}}\bigl((1-u)\ell(a,0)+u\ell(a,1)\bigr)$}; the function $H$ is
...
\ReviewDel{none of that}\ReviewAdd{none of those assumptions}.
...
dictionary \ReviewDel{doing work the original framework leaves implicit}\ReviewAdd{separating clauses that the original framework states together}. Reid and
...
This is the \ReviewDel{seductive}\ReviewAdd{tempting} misclassification of
Section~\ref{ssec:diagnostic} \ReviewDel{in the flesh}\ReviewAdd{in miniature}: the saturated identity
...
shape is \ReviewDel{the tent\,---\,and the tent is not merely an example.
The tent is not an example chosen for convenience; it is forced, and its}\ReviewAdd{piecewise affine; with the usual $0$--$1$ endpoint and scale normalizations, this becomes the familiar symmetric tent, whose}
...
\emph{converse}: piecewise saturation on the benchmark cells pins \ReviewDel{exactly the
tent shape}\ReviewAdd{the piecewise-affine shape; the endpoint and scale normalizations then select the tent}, with affineness on each cell and continuity at the apex obtained
for free and no regularity assumed. The mixture representation is equally
concrete. \ReviewDel{Every}\ReviewAdd{Under the usual binary proper-score representation, every suitably normalized} proper score's uncertainty function $G$ is recovered from the
...
$\int_0^1 T_\theta\,d\theta = u(1-u)$ \ReviewAdd{with this normalization} is then the Brier entropy, the case
...
\node[anchor=west] at (0.32,0.14) {\scriptsize concave $G$: $U_G$ \ReviewDel{falls}\ReviewAdd{is nonincreasing} (resolution gained)};
...
\node at (11.62,1.12) {\scriptsize $u(1-u)=\int_0^1\!T_\theta\,d\theta$ \ReviewAdd{(chosen norm.)}};
% bold tent
...
scores~\cite{Schervish1989,EGJK2016}, and \ReviewDel{every admissible
uncertainty function is a mixture}\ReviewAdd{the usual binary proper-score representation writes suitably normalized uncertainty functions as mixtures} $\int_0^1 T_\theta\,d\nu(\theta)$;
the dashed curve is the Brier entropy $u(1-u)$, the uniform mixture \ReviewAdd{under the chosen tent normalization}.
Exactness on each cell of the benchmark partition
$\{[0,\tfrac12],[\tfrac12,1]\}$ forces affineness on each cell
(Corollary~\ref{cor:piecewise}): \ReviewDel{the tent shape}\ReviewAdd{a piecewise-affine tent shape, with the familiar symmetric tent selected by endpoint and scale normalizations}, pinned with nothing
assumed at the kink.}
\label{fig:resolution}
\end{figure}

\paragraph{Two readings.}
\ReviewDel{The construction travels.}\ReviewAdd{The same reading travels.} In probabilistic graphical models, a clique
...
\ReviewDel{any genuinely concave~$G$ certifies, through a strict drop of~$U_G$,
that the representation gained resolution}\ReviewAdd{a genuinely concave~$G$ can certify a resolution gain through a strict drop of~$U_G$, provided the refinement crosses a segment on which $G$ is strictly concave}.
...
\ReviewDel{whole of}\ReviewAdd{main contrast of} Sections~\ref{sec:dictionary} and~\ref{sec:recurrence} is
...
coarse-graining is \ReviewDel{finite/rational mixing}\ReviewAdd{a finite-additivity operation rather than a direct continuum of real convex mixtures}\,---\,the first source.
...
predicts \ReviewDel{exactly where each route must spend a regularity hypothesis, and the
derivations cited above spend it exactly there}\ReviewAdd{which route needs a rational-to-real bridge, and the derivations cited above supply such a bridge through positivity, boundedness, or continuity-type hypotheses}.
...
\midrule
\textbf{Continuity}
  & \ReviewDel{\textbf{Required}}\ReviewAdd{\textbf{Load-bearing}} (Faddeev~\cite{Faddeev1957}).
  & \textbf{Vestigial}\,---\,a conclusion (Corollary~\ref{cor:regularity}). \\
\midrule
\textbf{Measurability}
  & \ReviewDel{\textbf{Required}}\ReviewAdd{\textbf{Load-bearing}} (Lee~\cite{Lee1964}).
  & \textbf{Vestigial}\,---\,same. \\
\midrule
\textbf{Monotonicity}
  & \ReviewDel{\textbf{Required}}\ReviewAdd{\textbf{Load-bearing}} (Kendall~\cite{Kendall1964}, Borges~\cite{Borges1967}).
  & \textbf{Vestigial}\,---\,same. \\
\midrule
\textbf{Boundedness on a set of positive measure}
  & \ReviewDel{\textbf{Required}}\ReviewAdd{\textbf{Load-bearing}} (Diderrich~\cite{Diderrich1975}).
  & \textbf{Vestigial}\,---\,same. \\
\midrule
\textbf{Boundedness on the interval}
  & \ReviewDel{\textbf{Required}}\ReviewAdd{\textbf{Load-bearing}} (a fortiori).
  & \textbf{Vestigial}\,---\,same. \\
\midrule
\textbf{None}
...
same five hypotheses \ReviewDel{are indispensable}\ReviewAdd{serve as load-bearing regularity routes} for the \emph{binary} ($n{=}2$)
...
The recurrence worth articulating is \ReviewDel{not a habit of carrying
unnecessary hypotheses; it is}\ReviewAdd{less a habit of carrying unnecessary hypotheses than} the ease of misreading which of the
...
and only the last retires the regularity hypotheses. A \ReviewDel{zeroth gate}\ReviewAdd{zeroth threshold}
...
of Section~\ref{ssec:diagnostic} is offered as \ReviewDel{the means of telling}\ReviewAdd{a way to tell}
...
Hamel pathology \ReviewDel{lives at}\ReviewAdd{survives rational tests and is exposed by irrational}~$p$; the continuous-coefficient
equation~\eqref{eq:star} forecloses on it there.} Why does this material
deserve an article in the \textsc{Monthly}\ReviewAdd{ as an expository matter}?
...
make visible. \ReviewDel{The present paper is the scaffolding.}\ReviewAdd{That scaffolding is the paper's expository purpose.}

\subsection*{A note on provenance.}

The present observation reached the author through an unconventional
route: a \ReviewDel{\emph{formalization project}}\ReviewAdd{formalization project} on a separate topic, in which the
...
A literature search then \ReviewDel{confirmed that the substitution technique is
folklore:}\ReviewAdd{placed the substitution technique in its folklore context:} Acz\'el's \cite[\S2.1.4]{Aczel1966} proves a dyadic precursor
...
The lesson is
\ReviewDel{general}\ReviewAdd{broader}: a defensive regularity hypothesis is not free, and a
