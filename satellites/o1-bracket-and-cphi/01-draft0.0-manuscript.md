...
### Refinement-lattice calibration via normalized concave scores

*Draft 0.0 (skeleton) — Executor T. SPL-format short paper, prepared for submission to* Statistics & Probability Letters. *Authored 2026-06-06.*
...
The classical Blackwell–Le Cam framework [3, 16, 25] studies a *fixed* loss and asks how the Bayes risk varies as the underlying *experiment* moves through the refinement order. The result is qualitative: a more informative experiment cannot raise the Bayes risk. It does not produce a quantitative bound from a scalar summary.

The Bartlett–Jordan–McAuliffe / Zhang / Reid–Williamson surrogate-calibration framework [1, 19, 26] studies a *fixed* experiment and asks how the $0$-$1$ excess risk of a *learned* predictor relates to its surrogate excess risk. The result is the $\psi$-transform: $\psi(R-R^\ast) \le R_\varphi - R_\varphi^\ast$, with $\psi$ the convexification of the gap $H^-(\eta)-H(\eta)$ between the unconstrained-prediction and the constrained-prediction conditional Bayes risks. This is the **optimization axis**: variation lives in the predictor; the experiment is held fixed.
...
The probability space is **atomless** if every measurable set of positive measure contains a measurable subset of strictly smaller positive measure. On an atomless space, Sierpiński's theorem [22] — every value in $[0,\mu(S)]$ is realised as the measure of some measurable subset of $S$ — gives the technical hypothesis under which arbitrary binary splits of cells of arbitrary masses and arbitrary conditional rates are realizable; see Halmos [11, §41] and Bogachev [4, Vol. 1, §1.12] for textbook treatments. The atomless hypothesis is needed for the converse direction of Theorem 1; the forward direction is unconditional.
...
The strict-monotonicity clause (6) is included for the existence of the inverse $\varphi^{-1}:[0,1]\to[0,\tfrac12]$, which the lower bracket below invokes. Without (6) the upper bracket of §5 still holds; the lower one needs an inverse and so needs strict monotonicity in $\varphi$.
...
**Remark 1.** Lemma 1 is a single-paragraph specialisation of the standard chord-domination property of concave functions (cf. Hardy–Littlewood–Pólya [12, §3.18] and Roberts–Varberg [20, Ch. 1]) to the normalised class. The novelty is not the inequality per se — it is the use we put it to in §5: the *uniformity* of $c_\varphi = \tfrac12$ across the class follows immediately, with the same single attaining point $\eta = \tfrac12$, regardless of which $\varphi$ is chosen.
...
|---|---|---|---|---|
| Shannon $\varphi_H$ | $-\tfrac14\log_2\tfrac14-\tfrac34\log_2\tfrac34\approx 0.8113$ | $0.8113$ | $0.4057$ | $\approx 0.2451$ |
| Variance $\varphi_{\mathrm{var}}$ | $4\cdot\tfrac14\cdot\tfrac34=\tfrac34$ | $0.75$ | $0.375$ | $0.25 - \tfrac14(\sqrt{1-3/4}\,)$ recovers $\eta\in\{\tfrac14,\tfrac34\}$ with $\eta(1-\eta) = 3/16$, so $\varphi_{\mathrm{var}}^{-1}(\tfrac34) = \tfrac14$ |
| Tent $T$ | $\tfrac12$ | $0.5$ | $0.25$ | $T^{-1}(\tfrac12) = \tfrac14$ |
| Hellinger $\varphi_q$ | $2\sqrt{\tfrac3{16}} = \tfrac{\sqrt 3}{2}\approx 0.8660$ | $\approx 0.8660$ | $\approx 0.4330$ | $\approx 0.25$ |

**Reading.** For the tent, both bounds equal $\tfrac14 = \varepsilon^\ast(\Pi)$ exactly — the bracket is tight. For the three smooth members the lower bracket equals $\tfrac14 = \varepsilon^\ast(\Pi)$ (because $\eta_1 = \tfrac14$ is realizable as a single-cell rate; cf. Corollary 1) but the upper bracket is strictly looser: $\tfrac12\bar\varphi_H(\Pi)\approx 0.406$, $\tfrac12\bar\varphi_{\mathrm{var}}(\Pi) = 0.375$, $\tfrac12\bar\varphi_q(\Pi)\approx 0.433$ — all strictly greater than $\tfrac14$. Among the three smooth members the rescaled variance gives the tightest upper bracket on this partition. (Pointwise, $\varphi_{\mathrm{var}}\le\varphi_H$ on $[0,1]$ — the standard inequality $4\eta(1-\eta)\le H(\eta)$ in nats; in bits the inequality reads $4\eta(1-\eta)\le -[\eta\log_2\eta+(1-\eta)\log_2(1-\eta)]$, valid on $[0,1]$ — so variance always gives a tighter upper bracket than entropy, on every partition.)

### 5.2 The bracket gap is the resolution-axis image of the surrogate gap

The pattern displayed in §5.1 — smooth members carry strictly positive bracket slack, the tent is exact — is the resolution-axis image of the surrogate-calibration phenomenon that smooth proper losses carry a strictly convex $\psi$-transform [1, 26]. The binary single-cell-pinning argument (Corollary 1) is the resolution-axis counterpart of "$\psi$ is the identity iff the loss is the $0$-$1$ loss" in the optimization-axis formulation. We elaborate on the correspondence in §6.
...
Le Cam's theory [16, 17] refines Blackwell with the **deficiency** $\delta(\mathcal E_1, \mathcal E_2)$ — the cost of using $\mathcal E_2$ in place of $\mathcal E_1$ for the worst decision problem. This is a quantitative refinement of Blackwell, but it parametrises the *experiment*, not the *scoring functional*; the analogue of our $c_\varphi$ is not a Le Cam construct.

The Theorem 1 **converse direction** — that binary-split monotonicity *forces* $\varphi$ concave, so the class of admissible scoring functionals on the partition lattice is exactly the concave class — is the natural converse to Blackwell on the partition-lattice axis. We do not find it stated explicitly in Blackwell [3], Le Cam [16, 17], Torgersen [25], or the subsequent comparison-of-experiments literature [9, 18]. It is implicit in any derivation of the proper-loss / generalised-entropy correspondence (see §6.3 below), but the *individual binary split* characterisation — which is the load-bearing element for the partition-lattice reading — is what we extract.
...
$$
relating excess $0$-$1$ risk of a learned predictor $\hat f$ to its excess surrogate risk, with the experiment held fixed throughout. Zhang [26] develops a parallel result via convexification of the loss directly. Reid and Williamson [19] unify these and the underlying f-divergence / Bregman / proper-loss / scoring-rule families into a single calibrated framework for binary experiments; their development (especially [19, §3, §4]) is the closest single source of the convex-analytic toolkit used in the present note.
...
| Lower instrument | (typically not given) | $\varphi^{-1}(\bar\varphi(\Pi))$ |
...
$$
is concave on $[0,1]$ [Savage 21; DeGroot 6; Grünwald–Dawid 10]. Every concave $H$ vanishing at the endpoints arises in this way, so the proper-loss / generalised-entropy correspondence gives the bijection between proper losses and (a subclass of) the concave $\varphi$ of this note. Specifically: our normalised class corresponds to proper losses for which $H$ is normalised by $H(\tfrac12) = 1$ and is symmetric — a mild canonical normalisation.

Under the correspondence, our partition functional $\bar\varphi(\Pi)$ coincides with the partition-restricted Bayes risk of the matched proper loss: $\bar\varphi(\Pi) = \sum_i p_i H(\eta_i)$ is the optimal expected loss over predictors constant on the cells of $\Pi$. The *lower* endpoint of our bracket (Theorem 3) is essentially the Jensen-via-tower-property bound on this matched-loss expectation — a fact that, after the proper-loss correspondence, can be read off from DeGroot [6, Ch. 14] or Grünwald–Dawid [10, §3].

The *upper* endpoint of our bracket — $\varepsilon^\ast(\Pi) \le \tfrac12 \bar\varphi(\Pi)$ with the uniform $c_\varphi = \tfrac12$ — has no analogue in DeGroot [6] or Grünwald–Dawid [10]: those works treat the matched-loss Bayes risk as a primitive object, not as an upper bound on the $0$-$1$ Bayes risk for a different loss. The chord-domination Lemma 1 — which controls the $0$-$1$ Bayes risk by *every* normalised concave functional uniformly — is the new piece.
...
The kernel theorems carry the following names:

- `NormalizedScore φ` — the structure recording clauses (1)–(6) of §2.3 on a function `φ : ℝ → ℝ`.
- `two_eta_le_of_normalized` — Lemma 1 of this note (chord domination $\varphi(\eta)\ge 2\eta$ on $[0,\tfrac12]$).
- `cPhi_eq_half_of_normalized` — Theorem 2 of this note ($c_\varphi = \tfrac12$ uniformly).
- `barPhi_refinement_le` — the partition-additivity workhorse for the forward direction of Theorem 1, including the dimension-agnostic refinement infrastructure (`refining`, `sum_cellMass_refining_eq`, `cellRate_mul_cellMass_refining_sum`) developed in Phase C2 of the parent project.
- `theorem1_easy` — Theorem 1, $(\mathrm{iii})\Rightarrow(\mathrm{i})\Rightarrow(\mathrm{ii})$ direction.
- `theorem1_hard` — Theorem 1, $(\mathrm{ii})\Rightarrow(\mathrm{iii})$ direction (the converse to Blackwell).
- `theorem1` — the combined equivalence.
- `bracket_lower` and `bracket_upper` — the two endpoints of Theorem 3.

The Lean development consumes two **realizability typeclasses** — `SingleCellRealizable` (atomless realization of a single-cell partition of arbitrary rate) and `BinarySplitRealizable` (atomless realization of a binary split with arbitrary subcell rates) — each provable from atomlessness via Sierpiński's theorem [22]. (Sierpiński's theorem is not currently in mathlib v4.29.1; a contribution is in preparation in a sibling branch of the parent project.) The general-space density argument is recovered from the atomless statement at the manuscript level (this note's §4); the Lean kernel verifies the atomless case directly.

The Lean source is available alongside the parent project; the relevant files are `Rigidity/Bracket.lean` (Definitions, Lemma 1, Theorems 2 and 3) and `Rigidity/Theorem1.lean` (Theorem 1, both directions). The `Audit/PrintAxioms.lean` file confirms the axiom-cleanness of each theorem.

The mechanisation is not load-bearing for the present note — every proof here is given in full text — but it provides an independent check on the analytic claims.
...
4. Bogachev, V. I. *Measure Theory.* 2 vols., Springer, 2007.
5. Buja, A., Stuetzle, W., and Shen, Y. *Loss functions for binary class probability estimation and classification: structure and applications.* Technical report, University of Pennsylvania, 2005.
6. DeGroot, M. H. *Uncertainty, information, and sequential experiments.* Ann. Math. Statist. **33** (1962), 404–419.
7. DeGroot, M. H. *Optimal Statistical Decisions.* McGraw–Hill, 1970.
8. Devroye, L., Györfi, L., and Lugosi, G. *A Probabilistic Theory of Pattern Recognition.* Springer, 1996.
9. Goel, P. K., and DeGroot, M. H. *Comparison of experiments and information measures.* Ann. Statist. **7** (1979), 1066–1077.
10. Grünwald, P. D., and Dawid, A. P. *Game theory, maximum entropy, minimum discrepancy, and robust Bayesian decision theory.* Ann. Statist. **32** (2004), 1367–1433.
11. Halmos, P. R. *Measure Theory.* Springer, 1974 (corrected reprint of 1950 edition).
12. Hardy, G. H., Littlewood, J. E., and Pólya, G. *Inequalities.* 2nd ed., Cambridge University Press, 1952.
13. Hu, W., Niu, G., Sato, I., and Sugiyama, M. *Does distributionally robust supervised learning give robust classifiers?* In Proc. ICML, 2018. (Cited only as a reference application of $\psi$-transform thinking; not load-bearing for any claim here.)
14. Jensen, J. L. W. V. *Sur les fonctions convexes et les inégalités entre les valeurs moyennes.* Acta Math. **30** (1906), 175–193.
15. Kullback, S. *Information Theory and Statistics.* Wiley, 1959.
16. Le Cam, L. *Asymptotic Methods in Statistical Decision Theory.* Springer, 1986.
17. Le Cam, L., and Yang, G. L. *Asymptotics in Statistics: Some Basic Concepts.* 2nd ed., Springer, 2000.
18. Liese, F., and Vajda, I. *On divergences and informations in statistics and information theory.* IEEE Trans. Inform. Theory **52** (2006), 4394–4412.
19. Reid, M. D., and Williamson, R. C. *Information, divergence and risk for binary experiments.* J. Mach. Learn. Res. **12** (2011), 731–817.
20. Roberts, A. W., and Varberg, D. E. *Convex Functions.* Academic Press, 1973.
21. Savage, L. J. *Elicitation of personal probabilities and expectations.* J. Amer. Statist. Assoc. **66** (1971), 783–801.
22. Sierpiński, W. *Sur les fonctions d'ensemble additives et continues.* Fund. Math. **3** (1922), 240–246.
23. Steinwart, I. *How to compare different loss functions and their risks.* Constr. Approx. **26** (2007), 225–287.
24. Tewari, A., and Bartlett, P. L. *On the consistency of multiclass classification methods.* J. Mach. Learn. Res. **8** (2007), 1007–1025.
25. Torgersen, E. *Comparison of Statistical Experiments.* Cambridge University Press, 1991.
26. Zhang, T. *Statistical behavior and consistency of classification methods based on convex risk minimization.* Ann. Statist. **32** (2004), 56–85.
