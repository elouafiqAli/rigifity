...
measurements, and coarse-graining is finite mixing\,---\,the first
...
theorem~\cite{Gleason1957} and in Busch's effect-algebra
version~\cite{Busch2003}, positivity or an explicit continuity proof
in Caves, Fuchs, Manne, and Renes~\cite{CFMR2004}, and, in the
Cauchy-equation treatment of Wright and
Weigert~\cite{WrightWeigert2019}, the catalog that nonlinear frame
functions can be bounded on neither side, continuous at zero, nor
Lebesgue measurable\,---\,the catalog of Figure~\ref{fig:dictionary}
rediscovered inside physics. Preparation tests the identity at every
real~$p$; coarse-graining tests it only at rationals. The dictionary
predicts exactly where each route must spend a hypothesis, and the
