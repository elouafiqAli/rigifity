...
$D^{\ast}:=\bigcup_{\lambda>0}\lambda D$ define
$\hat A(\lambda v):=\lambda\,A(v)$ for $v\in D$, $\lambda>0$. This is
...
basis exists by Zorn's lemma applied to the partially ordered set of
$\Q$-linearly independent subsets of~$\R$ that contain~$\{1\}$. Pick
...
$h\in(0,1)$.\footnote{Cardinality alone does not place a basis element
in $(0,1)$: rescaling every element of a Hamel basis into $[1,2]$, say,
produces a basis that avoids $(0,1)$ entirely. The rational-rescaling
...
in~$\R$.} Define a $\Q$-linear map $\ell\colon\R\to\R$ by setting
